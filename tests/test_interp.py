...
# # TEST interp.ll2xe for multiple lon0/lat0 shapes and for inputting user dims
# # make sure this all works whether extra grid has been set up or not
# # try to interpolate with z0s without having called roms_dataset yet

# MOVE TESTS HERE FROM NOTEBOOK AND MAKE SURE THEY WORK
def test_interp_1coord():
    ...
    varout = xroms.interp.interp(varin, tris['u'], lon0, lat0)
...
def test_interp_2coord():
    '''Test interp with multiple xy locations and depths.'''
    ...
    varout = xroms.interp.interp(varin, tris['u'], lon0, lat0, z0s)
    assert np.allclose(varout, varin.isel(indexer))    
...
def test_interp_triplets():
    '''Test interp with triplets.'''
    ...
    varout = xroms.interp.interp(varin, tris['u'], lon0, lat0, z0s, triplets=True)
