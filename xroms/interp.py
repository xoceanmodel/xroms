...
def interpll(var, lons, lats, which='pairs'):
    ...
    var = var.cf.transpose(*[dim for dim in ["T", "Z", "Y", "X"] if dim in var.cf.get_valid_keys()])
    ...
    zkey_varint = [coord for coord in varint.coords if 'z_' in coord and '0' not in coord]
    
    # get z coordinates to go with interpolated output if not available
    if zkey_varint == []:    
        zkey = [coord for coord in var.coords if 'z_' in coord and '0' not in coord][0]  # str
    ...
    if which == 'pairs':
        varint['locations'] = ('locations', np.arange(varint.sizes['locations']), {'axis': 'X'})
    elif which == 'grid':
...
def isoslice(var, iso_values, grid=None, iso_array=None, axis='Z'):
    ...
    * on w grid:
    ...
    assert (grid is not None) or (isinstance(var, xr.DataArray) and 'grid' in var.attrs), words
    ...
    if iso_array is None:
        key = [coord for coord in var.coords if 'z_' in coord and '0' not in coord][0]  # str
        assert len(key)>0, 'z coordinates associated with var could not be identified.'
        iso_array = var[key]
    else:
    ...
    if zkey not in transformed.coords:
        transformedZ = grid.transform(var[zkey], axis, iso_values, target_data=iso_array)    
    ...
    if latkey not in transformed.coords:
        ...
        if 'T' in iso_array.cf.get_valid_keys():
            iso_array = iso_array.cf.isel(T=0).drop_vars(iso_array.cf['T'].name, errors='ignore')
        if 'Z' in iso_array.cf.get_valid_keys():
            iso_array = iso_array.cf.isel(Z=0).drop_vars(iso_array.cf['Z'].name, errors='ignore')
        transformedlat = grid.transform(var[latkey], axis, iso_values, target_data=iso_array) 
    ...
    if lonkey not in transformed.coords:
        ...
        if 'Z' in iso_array.cf.get_valid_keys():
            iso_array = iso_array.cf.isel(Z=0).drop_vars(iso_array.cf['Z'].name, errors='ignore')
        transformedlon = grid.transform(var[lonkey], axis, iso_values, target_data=iso_array)
    ...
    transformed = transformed.cf.transpose(*[dim for dim in ["T", "Z", "Y", "X"] if dim in transformed.cf.get_valid_keys()])
