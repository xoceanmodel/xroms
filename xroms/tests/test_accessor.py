...
def test_speed():
    ...
    axes = ['T','X','Y','Z']
    coords = ['longitude','latitude','vertical','time']
    assert set(axes).issubset(acc.cf.axes)
    assert set(coords).issubset(acc.cf.coords)
    ...
    for testvar in testvars:
        for scoord in ['s_w','s_rho']:
            for hcoord in ['rho','u','v','psi']:
                ...
                assert np.allclose(acc, xroms.to_grid(ds[testvar], grid, hcoord=hcoord, scoord=scoord))
                acc.name == acc.attrs['name']
                acc.attrs['grid'] == ds.xroms.grid
                axes = ['T','X','Y','Z']
                coords = ['longitude','latitude','vertical','time']
...
def test_sel2d():
    ...
    for testvar in testvars:
        ...
        out = xroms.sel2d(ds[testvar], ds[testvar].cf["longitude"], 
...
def test_interpll():
    ...
    testvars = ['salt', 'u', 'v', 'z_w']
    for testvar in testvars:
        var1 = xroms.interpll(ds[testvar], ds.lon_rho.isel(indexer), 
                              ds.lat_rho.isel(indexer))
        var2 = ds[testvar].xroms.interpll(ds.lon_rho.isel(indexer), 
                                          ds.lat_rho.isel(indexer))
        assert np.allclose(var1, var2)
...
def test_zslice():
    ...
    for testvar in testvars:   
        ...
        assert np.allclose(varout.cf.isel(T=0,Y=0,X=0), varcomp.cf.isel(T=0,Y=0,X=0))