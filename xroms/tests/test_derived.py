...
def test_KE():
    ...
    assert np.allclose(
        xroms.to_grid(xroms.KE(ds.rho0, s), grid, hcoord="psi").mean(), KE.mean(), rtol=1e-2
    )
...
def test_uv_geostrophic():
    ...
    assert np.allclose(xroms.uv_geostrophic(ds.zeta, ds.f, grid, which='xi')[0,0,0], ug)
